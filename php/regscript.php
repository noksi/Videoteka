...
if (isset($_POST['registracija']) && !empty($_POST['korisnik']) && !empty($_POST['lozinka'])) {
    $image=addslashes(file_get_contents($_FILES['profilnaslika']['tmp_name']));
$query="insert into login (username, password, avatar) values ('".$_POST['korisnik']."', '".$_POST['lozinka']."', '".$image."')";
